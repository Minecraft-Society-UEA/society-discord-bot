...
import { getProfileByBedMcUsername, getProfileByDId, getProfileByMcUsername } from '../../utill/database_functions'
...
export const config = createCommandConfig({
	...
	options: [
		{
			name: 'bedmc-username',
			description: 'bedrock mc username exactly how it is in the player list',
			type: 'string',
			required: true
		}
	]
...
export default async (
	...
	const username = options['bedmc-username']
	const embed = new EmbedBuilder()

	if (!username.startsWith(`.`))
		return { embeds: [embed.setTitle(`The username entered isnt a bedrock username`).setColor(`Red`)] }

	// fetch db profiles
	const did_profile = (await getProfileByDId(interaction.user.id)) as db_player | null
	const username_inuse = (await getProfileByMcUsername(username)) as db_player | null
	const already_verified = (await getProfileByBedMcUsername(username)) as db_player | null

	if (did_profile?.mc_username || !already_verified?.bed_mc_username || !username_inuse) {
		// pull hub players
		...
		await Flashcore.set(`verify_bed_code-${interaction.user.id}`, code)
		await Flashcore.set(`verify_code-bed_mc_username-${interaction.user.id}`, player.name)
		...
		const modal = new ModalBuilder()
			.setCustomId(`bed_mc-code-${interaction.user.id}`)
		...
		return {
			embeds: [
				embed
					.setColor('Yellow')
					.setTitle(`✦ You have'nt already successfully verified on Minecraft or that username is in use.`)
			]
		}
	}
}
