...
export default async (interaction: ChatInputCommandInteraction): Promise<CommandResult> => {
	...
	const modal = new ModalBuilder().setCustomId('modmail_modal_intro').setTitle('Customise the title and footer')
	...
	const one = new TextInputBuilder()
		...
		.setValue(setting.setting.introMsg.title ?? ``)
		.setRequired(true)
	...
	const two = new TextInputBuilder()
		...
		.setValue(setting.setting.introMsg.description ?? ``)
		.setRequired(true)
		.setStyle(TextInputStyle.Short)
	...
	const three = new TextInputBuilder()
		...
		.setValue(setting.setting.introMsg.footer ?? ``)
		.setRequired(true)
		.setStyle(TextInputStyle.Short)
